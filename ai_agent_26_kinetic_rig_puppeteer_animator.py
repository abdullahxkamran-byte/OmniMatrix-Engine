...
class KineticRigPuppeteerAnimator:
    def __init__(self, drive_temp_dir="G:/My Drive/ZNET_Temp", local_library_dir="D:/ZNET_Local_Assets", blender_path="blender"):
        self.agent_name = "Ai Agent 26: AAA Kinetic Rig Puppeteer"
        ...
        self.output_blueprint = os.path.join(self.env_dir, "26_animation_blueprint.json")
    ...
    def _query_animator_brain(self, scene_name, context):
        ...
        try:
            ...
            self.log_message(f"AI Animation Generation failed: {str(e)}. Using fallback.", "WARNING")
            return self._fallback_animation(context)
...
try:
    # 1. Find the Character Armature (Rig)
    armature = None
    ...
    if not armature:
        ...
        # 5. Set Interpolation Mode (Omni-Universal Style Handling)
    ...
    def process_stage_animations(self):
        ...
        for filename in os.listdir(self.env_dir):
            if filename.endswith("_stage.blend"):
                ...
                self.log_message(f"AI Decision: Applying '{anim_data['animation_rationale']}' | Interpolation: {anim_data['interpolation_type']}", "INFO")
                ...
                try:
                    ...
                    if result.returncode == 0:
                        self.log_message(f"Animation successfully injected into {filename}", "INFO")
